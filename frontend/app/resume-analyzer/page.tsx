...
export default function ResumeAnalyzerPage() {
    ...
    const handleDownloadReport = () => {
        ...
        doc.text(`${file.name} - ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}`, margin, y);
        y += 15;

        // Score
        const scoreColor = result.score >= 7 ? [34, 197, 94] : result.score >= 4 ? [234, 179, 8] : [239, 68, 68];
        doc.setFontSize(36);
        doc.setFont("helvetica", "bold");
        doc.setTextColor(scoreColor[0], scoreColor[1], scoreColor[2]);
        doc.text(`${result.score}/10`, margin, y);

        const scoreLabel = result.score >= 8 ? "Excellent" : result.score >= 6 ? "Good" : result.score >= 4 ? "Fair" : "Needs Work";
        doc.setFontSize(12);
        ...
        if (result.issues && result.issues.length > 0) {
            ...
            result.issues.forEach((issue) => {
                ...
                doc.setTextColor(34, 197, 94);
                const tipLines = doc.splitTextToSize(`Tip: ${issue.suggestion}`, contentWidth);
        ...
        doc.text("Generated by Shadow Instructor Resume Analyzer", pageWidth / 2, pageHeight - 10, { align: "center" });
            ...
            <main className="flex-1 flex pt-16">
                ...
                <div className="flex-1 border-r border-neutral-800 flex flex-col">
                    <div className="h-14 px-4 border-b border-neutral-800 flex items-center justify-between shrink-0">
                        <h2 className="text-sm font-medium text-neutral-400">Resume Preview</h2>
                        {file && (
                        ...
                        {pdfUrl ? (
                            ...
                            >
                                <div className="flex flex-col items-center justify-center h-full text-neutral-500">
                                    ...
                                    <a href={pdfUrl} target="_blank" rel="noopener noreferrer" className="text-white underline text-sm mt-2">
                                        Open in new tab
                                    </a>
                                </div>
                            </object>
                            ...
                            >
                                ...
                                <p className="text-neutral-300 font-medium mb-1">Upload Resume</p>
                    ...
                    <div className="flex-1 overflow-y-auto p-4">
                        {!file ? (
                            <div className="h-full flex flex-col items-center justify-center text-center">
                                ...
                                <p className="text-neutral-400 text-sm">Upload a resume to analyze</p>
                            </div>
                            ...
                            <div className="h-full flex flex-col items-center justify-center">
                                ...
                                <p className="text-sm text-neutral-400">Analyzing design...</p>
                            </div>
                        ) : error ? (
                            <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl">
                                ...
                                <button onClick={handleAnalyze} className="mt-3 text-xs text-white underline">
                                    Retry
                                </button>
                            </div>
                        ) : result ? (
                                ...
                                <div className="flex items-center gap-4 p-4 bg-neutral-800/50 rounded-xl border border-neutral-700">
                                    <div className="relative w-16 h-16 flex items-center justify-center">
                                        <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 64 64">
                                            <circle cx="32" cy="32" r="28" stroke="#262626" strokeWidth="4" fill="none" />
                                            <circle
                                                cx="32"
                                                cy="32"
                                                r="28"
                                                stroke={result.score >= 7 ? "#22c55e" : result.score >= 4 ? "#eab308" : "#ef4444"}
                                                strokeWidth="4"
                                                fill="none"
                                                strokeDasharray={176}
                                    ...
                                    <div className="flex-1">
                                        <h3 className="text-xs font-medium text-neutral-500 uppercase tracking-wider mb-1">Overall Score</h3>
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-1.5">
                                            {result.strengths.map((s, i) => (
                                                <div key={i} className="text-sm text-neutral-300 pl-5 relative before:content-['•'] before:absolute before:left-1.5 before:text-green-400">
                                                    {s}
                                                </div>
                                            ))}
                                        </div>
                                            ...
                                            {result.issues.map((issue, i) => (
                                                <div key={i} className="p-3 bg-neutral-800/50 border border-neutral-700 rounded-lg">
                                                    <div className="flex items-center gap-2 mb-1.5">
                                                        {categoryIcons[issue.category] || <AlertTriangle className="w-4 h-4" />}
                                                        <span className="text-xs font-medium text-neutral-400">{issue.category}</span>
                                                    </div>
                                                    <p className="text-sm text-neutral-200 mb-1.5">{issue.description}</p>
