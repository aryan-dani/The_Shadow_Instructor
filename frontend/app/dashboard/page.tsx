...
export default function DashboardPage() {
  ...
  const startEdit = (interview: Interview) => {
    ...
    setEditName(
      interview.custom_name || `${interview.job_role} - ${interview.topic}`,
    );
  };
  ...
  const avgScore = interviews.length
    ? Math.round(
        interviews.reduce((acc, i) => acc + (i.overall_score || 0), 0) /
          interviews.length,
      )
    : 0;
  ...
  const totalMinutes = interviews.reduce(
    (acc, i) => acc + Math.round((i.duration_seconds || 0) / 60),
    0,
  );
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            ...
            <div className="rounded-xl border border-neutral-800 bg-neutral-900/80 p-4">
              ...
              <div className="text-2xl font-semibold">
                {totalMinutes}
                <span className="text-base text-neutral-600"> min</span>
              </div>
            </div>
          </div>
